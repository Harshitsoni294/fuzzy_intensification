...
def BBHE_STD_ALGO(img):
    L = 256
    x = np.arange(L)
    w, l = img.shape
    len_img = w * l
    y = img.flatten()
    std_dev = int(np.round(np.std(img)))  # Use standard deviation instead of mean
    ...
    C_L = np.zeros(std_dev + 1)
    C_U = np.zeros(256 - (std_dev + 1))

    n_L = np.sum(Ihist[:std_dev + 1])
    n_U = np.sum(Ihist[std_dev + 1:])

    P_L = Ihist[:std_dev + 1] / n_L
    P_U = Ihist[std_dev + 1:] / n_U
    ...
    for r in range(w):
        for s in range(l):
            if img[r, s] < (std_dev + 1):
                f = std_dev * C_L[img[r, s]]
                BBHEoutput[r, s] = np.round(f)
            else:
                f = (std_dev + 1) + (255 - std_dev) * C_U[img[r, s] - (std_dev + 1)]
                BBHEoutput[r, s] = np.round(f)
...
img = cv2.imread('family.contrast.4.png', cv2.IMREAD_GRAYSCALE) 
result = BBHE_STD_ALGO(img)
