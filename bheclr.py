...
def bi_histogram_equalization(channel):
    mean_val = np.mean(channel)
    
    low_part = channel[channel <= mean_val]
    high_part = channel[channel > mean_val]
...
better_b = bi_histogram_equalization(b)
better_g = bi_histogram_equalization(g)
better_r = bi_histogram_equalization(r)

better_img = cv2.merge((better_b, better_g, better_r))

cv2.imwrite('better_image_bhe_color.jpg', better_img)
